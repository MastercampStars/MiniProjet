...
class Vehicule(Element):
    ...
    def getMatrice(self):
        ...
        matrice = [[self.type.copy() for i in range(self.size["x"])]
                   for j in range(self.size["y"])]
        for tourelle in self.tourelles:
            matrice[tourelle["y"]][tourelle["x"]]["char"] = "T"
            matrice[tourelle["y"]][tourelle["x"]
    ...
    def move(self, direction: str):
        ...
        newVehicule = Vehicule(self.type, self.map,
                               newFront, newDirection, self.size)
...
class BigBoat (Vehicule):
    ...
    tourelles1 = [{"x": 0, "y": 1, "direction": "left"}, {"x": 0, "y": 5, "direction": "left"}, {"x": 0, "y": 10, "direction": "left"}, {
        "x": 2, "y": 1, "direction": "right"}, {"x": 2, "y": 5, "direction": "right"}, {"x": 2, "y": 10, "direction": "right"}]
    tourelles2 = [{"x": 0, "y": 0, "direction": "up"}, {"x": 1, "y": 0, "direction": "up"}, {"x": 2, "y": 0, "direction": "up"}, {
        "x": 0, "y": 12, "direction": "down"}, {"x": 1, "y": 12, "direction": "down"}, {"x": 2, "y": 12, "direction": "down"}]
...
class Carrier (Vehicule):
    ...
    def special(self):
        self.map.addElement(Jet(self.map, {
                            "x": self.Front["x"], "y": self.Front["y"]}, self.direction, self.type["player"], self.type["color"]))
